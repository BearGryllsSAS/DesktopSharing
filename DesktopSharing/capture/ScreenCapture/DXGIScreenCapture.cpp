...
初始化
	...
	通过DX::GetMonitors获取显示器列表，选择指定索引的显示器。
	创建D3D11设备、DXGI工厂、适配器和输出。
	调用DuplicateOutput初始化输出复制接口。
	创建共享纹理及相关资源。
	启动捕获线程。
*/
...
/*
​后台线程
	循环调用AcquireFrame，间隔10ms以平衡性能与实时性。
*/
...
帧捕获核心
	...
	调用AcquireNextFrame获取最新桌面帧。
	复制帧到gdi_texture_，使用GDI绘制光标。
	将处理后的图像复制到rgba_texture_并映射到CPU内存。
	将像素数据从GPU复制到image_ptr_。
	通过键控互斥同步共享纹理的访问。
