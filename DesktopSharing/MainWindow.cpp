...
/*
根据渲染器名称选择合适的渲染驱动（OpenGL 或 Direct3D）。
创建渲染器并初始化它。
根据渲染器类型初始化一个 Overlay 对象。
设置覆盖层的显示区域，并注册观察者。
该函数综合了 SDL2 渲染器的初始化过程，支持硬件加速并能够灵活地处理不同的渲染驱动（OpenGL 或 Direct3D）。
*/
...
/*
将从 OnPaint 中获取到的 BGRA 图像数据渲染到窗口上，并处理纹理的创建、更新以及显示。
*/
